...
test.beforeAll(async () => {
  ...
  await writeFile(join(musicDirectory, 'fixture.wav'), createWavFixture());
...
test('login, index, play, search, and logout', async ({ page }) => {
  ...
  await test.step('stream, play, and pause the indexed song', async () => {
    ...
    await expect(page.getByText('Current song:').locator('..')).toContainText('fixture.wav');
